...
export default function ManagerTickets() {
  ...
  const handleFilterChange = (newFilters) => { setFilters(newFilters); fetchTickets(1, newFilters, activeTab); };
  const handlePageChange = (page) => { fetchTickets(page, filters, activeTab); };
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      {/* HEADER SECTION */}
      <div className="bg-gradient-to-r from-teal-600 to-emerald-700 rounded-3xl p-8 text-white shadow-lg flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black tracking-tight mb-2">Gestion des Tickets</h1>
          <p className="text-teal-50 text-sm font-medium">Visualisez, créez et suivez l'assistance de votre équipe.</p>
        </div>
        <button
          onClick={() => setShowModal(true)}
          className="shrink-0 bg-white text-emerald-600 hover:bg-emerald-50 px-6 py-3.5 rounded-xl text-sm font-black uppercase tracking-wider flex items-center gap-2 shadow-sm transition-all transform hover:scale-105"
        >
      ...
      <div className="flex bg-gray-100/50 p-1 rounded-2xl w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${
            activeTab === 'actifs' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'
          }`}
        >
      ...
      <FilterBar 
        ...
        personPlaceholder="Filtrer par technicien"
        statusOptions={
      ...
      <div className="overflow-x-auto pb-4">
        {loading ? (
          <div className="py-20 flex flex-col items-center justify-center text-gray-400 gap-3">
             ...
             <p className="font-medium">Chargement des tickets...</p>
          </div>
        ) : tickets.length === 0 ? (
          <div className="bg-white border border-gray-100 rounded-3xl py-20 flex flex-col items-center justify-center text-center shadow-sm">
            ...
            <p className="text-gray-400 text-sm">Il n'y a aucun ticket correspondant à vos critères.</p>
          </div>
        ) : (
          <table className="w-full text-left border-separate border-spacing-y-3 min-w-[900px]">
            <thead>
              <tr className="text-xs uppercase tracking-widest text-gray-400 font-bold px-4">
                ...
                <th className="px-6 py-2 font-semibold">Technicien assigné</th>
            ...
            <tbody>
              {tickets.map(ticket => (
                <tr key={ticket.id} className="bg-white group hover:shadow-md transition-all duration-300">
                  <td className="p-4 rounded-l-2xl border-y border-l border-gray-100 group-hover:border-emerald-200">
                  ...
                  <td className="px-6 py-4 border-y border-gray-100 group-hover:border-emerald-200 text-center">
                    {prioriteBadge(ticket.priorite)}
                  </td>
                  <td className="px-6 py-4 border-y border-gray-100 group-hover:border-emerald-200">
                  ...
                  <td className="px-6 py-4 rounded-r-2xl border-y border-r border-gray-100 group-hover:border-emerald-200 text-right pr-6">
                    <button
                      onClick={() => setSelectedTicketId(ticket.id)}
                      className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 hover:bg-emerald-600 hover:text-white transition-colors"
                      title="Voir détails"
                    >
          ...
          <div className="bg-white w-full max-w-xl rounded-[2rem] shadow-2xl overflow-hidden transform transition-all">
            ...
            <form className="p-8 space-y-5" onSubmit={handleSubmit}>
              <div>
                ...
                <input type="text" value={form.titre} onChange={(e) => setForm({ ...form, titre: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-medium focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all" placeholder="Ex: Remplacement de matériel" required />
              </div>
              <div className="grid grid-cols-2 gap-5">
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Catégorie</label>
                  <select value={form.categorie} onChange={(e) => setForm({ ...form, categorie: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-medium focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all">
                ...
                <div>
                  ...
                  <select value={form.technicien_id} onChange={(e) => setForm({ ...form, technicien_id: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-medium focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all">
              ...
              <div>
                ...
                <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-medium focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 outline-none transition-all resize-none" placeholder="Décrivez le problème en détail..." rows="4" required></textarea>
              </div>
              <button type="submit" disabled={isSubmitting} className="w-full bg-emerald-600 text-white py-4 rounded-xl text-sm font-black uppercase tracking-widest hover:bg-emerald-700 hover:shadow-lg transition-all mt-4 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                {isSubmitting ? <><span className="material-symbols-outlined animate-spin text-[18px]">autorenew</span> Envoi...</> : 'Créer le ticket'}
              </button>
            </form>
          </div>
        </div>
      ...
      {selectedTicketId && (
        <TicketDetailModal ticketId={selectedTicketId} role={2} onClose={() => setSelectedTicketId(null)} onUpdated={() => fetchTickets(currentPage, filters, activeTab)} />
      )}
    </div>
  );
}