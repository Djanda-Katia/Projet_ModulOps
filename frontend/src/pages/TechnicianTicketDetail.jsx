...
export default function TechnicianTicketDetail() {
  ...
  const ticketData = {
    1: {
      ...
      description: "Le VPN ne répond plus pour l'utilisateur AL.",
    ...
    2: {
      ...
      description: "Le logiciel RH est bloqué en version 2.0.",
    ...
    3: {
      id: 3,
      titre: "Accès Imprimante Bureau 4",
      description: "L'imprimante du bureau 4 ne s'allume plus.",
      categorie: "Matériel",
      priorite: "Basse",
      statut: "Fermé",
      demandeur: "SC",
    ...
    4: {
      id: 4,
      titre: "Demande Nouveau Poste Fixe",
  ...
  // Si le statut est "Fermé", on empêche toute modification et on cache le formulaire
  const isClosed = status === "Fermé";

  // Déterminer les options du menu déroulant selon le statut actuel
  const getStatusOptions = (currentStatus) => {
    if (currentStatus === "Ouvert") return ["En cours", "Résolu"];
    if (currentStatus === "En cours") return ["Résolu"];
    return []; // Résolu ou Fermé → aucune option
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className={`px-2 py-0.5 rounded text-xs font-bold uppercase ${
          status === "Ouvert" ? "bg-blue-100 text-blue-700" :
          status === "En cours" ? "bg-amber-100 text-amber-700" :
          status === "Résolu" ? "bg-green-100 text-green-700" :
          "bg-gray-100 text-gray-600"
        }`}>
          {status}
        </span>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div className="space-y-4">
              <div className="flex gap-4">
                <div className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-xs font-bold">{ticket.demandeur}</div>
                <div className="flex-1">
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg rounded-tl-none text-sm text-gray-700">
                    J'ai essayé de redémarrer le routeur local, mais cela n'a rien changé.
                  </div>
                </div>
              </div>
              <div className="flex gap-4 flex-row-reverse">
                ...
                <div className="flex-1">
                  ...
                  <div className="bg-blue-50 p-3 rounded-lg rounded-tr-none text-sm text-gray-700">
                    Bonjour Marie, je prends en charge votre ticket. Je reviens vers vous d'ici 30 minutes.
                  </div>
                </div>
              </div>
            </div>

            {/* Zone d'ajout de commentaire : cachée si le ticket est Fermé */}
            {!isClosed && (
                  ...
                  <div className="flex gap-4 items-end">
                    <div className="flex-1">
                      ...
                      <select
                        ...
                        className="w-full border border-gray-200 rounded-lg px-4 h-10 text-sm outline-none"
                    ...
                    <button 
                      className="h-10 px-6 bg-blue-600 text-white rounded-lg text-sm font-bold uppercase hover:bg-blue-700"
                      disabled={statusOptions.length === 0}
                    >
