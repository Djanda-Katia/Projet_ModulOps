...
export default function ManagerTicketDetail() {
  ...
  // Charger le ticket UNE SEULE FOIS
  useEffect(() => {
  ...
  }, []); // <-- Dépendances vides : un seul chargement
  ...
  // Ajouter commentaire (seulement pour technicien)
      ...
      {showBanner && ticket.statut === "Résolu" && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-5 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <button onClick={handleConfirm} className="flex-1 md:flex-none px-6 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-bold rounded-lg transition-all uppercase shadow-sm">
              CONFIRMER LA RÉSOLUTION
            </button>
            <button onClick={handleSignal} className="flex-1 md:flex-none px-6 py-2 border border-red-200 text-red-600 bg-white text-sm font-bold rounded-lg hover:bg-red-50 transition-all uppercase">
                  ...
                  ticket.commentaires.map((c) => (
                    <div key={c.id} className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="bg-gray-50 p-3 rounded-lg rounded-tl-none text-sm text-gray-700">{c.contenu}</div>
                      </div>
                    </div>
                  ))
                ) : (
              ...
              {ticket.technicien_id === user?.id && (
                <div className="mt-6 pt-6 border-t border-gray-100">
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      <textarea value={comment} onChange={(e) => setComment(e.target.value)} className="w-full border border-gray-200 rounded-lg p-3 text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none" rows="2" placeholder="Ajouter un commentaire..."></textarea>
                      <div className="mt-2 flex justify-end">
                        <button onClick={handleAddComment} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-bold transition-colors uppercase">Envoyer</button>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-gray-900">{ticket.technicien ? `${ticket.technicien.prenom} ${ticket.technicien.nom}` : "Non assigné"}</span>
                </div>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
                <span className="text-sm text-gray-500">Date de création</span>
                <span className="text-sm text-gray-900">{new Date(ticket.created_at).toLocaleDateString()}</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
